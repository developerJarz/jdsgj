...
export default function ProductDetailClient({
  ...
  const [activeTab, setActiveTab] = useState<
    'description' | 'how_to_use' | 'ingredients' | 'faq'
  >('description');
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-10">
      ...
      >
        <Link href="/" className="hover:text-sg-pink">
          Home
        </Link>
        <span>/</span>
        <Link
          href={`/shop?category=${product.category_slug}`}
          className="hover:text-sg-pink"
        >
          {product.category}
        </Link>
        <span>/</span>
        ...
        <span>/</span>
        <span className="text-sg-black font-semibold truncate max-w-xs">
          {product.name}
        </span>
      </nav>
              ...
              {product.images.map((img, idx) => (
                <button
                  ...
                  className={`relative w-16 h-16 rounded-xl overflow-hidden bg-gray-50 flex-shrink-0 border-2 transition-all ${
                    selectedImage === img
                      ? 'border-sg-pink ring-2 ring-sg-pink-light'
          ...
          <div>
            ...
            <div className="flex items-center gap-3 mt-2 text-xs">
              ...
              <span className="text-gray-400">
                ({product.reviews_count || 24} Verified Reviews)
              </span>
              <span className="text-gray-300">|</span>
              <span className="text-gray-500">
                SKU: <strong>{product.sku || `SG-${product.id}`}</strong>
              </span>
            </div>
          </div>
          ...
          <div className="p-4 rounded-xl bg-sg-pink-light/30 border border-sg-pink-border/40 flex items-baseline justify-between">
            <div>
              <div className="flex items-baseline gap-2">
                <span className="text-2xl font-black text-sg-pink">
                  ৳{product.sale_price}
                </span>
              ...
              <p className="text-[11px] text-emerald-700 font-semibold mt-0.5">
                <span className="flex items-center gap-1">
                  <GiftIcon className="w-3.5 h-3.5 text-emerald-600" /> Earn{' '}
                  <strong>{product.reward_points}</strong> Shajgoj.bd Reward
            ...
            <div className="text-right">
              <span className="inline-block px-2.5 py-1 rounded-full text-xs font-bold bg-emerald-100 text-emerald-800">
                <span className="flex items-center gap-1">
                  <CheckIcon className="w-3.5 h-3.5" /> In Stock (
                  {product.stock})
                </span>
              </span>
            </div>
          ...
          <button
            ...
            className={`pb-3 border-b-2 transition-colors whitespace-nowrap ${
              activeTab === 'description'
                ? 'border-sg-pink text-sg-pink'
                : 'border-transparent text-gray-500 hover:text-sg-black'
            }`}
          >
            Description
          </button>
            ...
            <div className="space-y-3">
              <div className="border border-gray-100 p-3 rounded-lg bg-gray-50">
                ...
                <p className="text-gray-600 mt-1">
                  Yes! Shajgoj.bd guarantees 100% authenticity on all products
                  sourced straight from authorized distributors.
                </p>
              </div>
              <div className="border border-gray-100 p-3 rounded-lg bg-gray-50">
                <p className="font-bold text-sg-black">
                ...
                <p className="text-gray-600 mt-1">
                  Inside Dhaka deliveries typically arrive within 24-48 hours.
                  Outside Dhaka orders arrive within 48-72 hours.
                </p>
              </div>
            </div>
          )}
